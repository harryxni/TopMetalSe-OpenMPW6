...
proc tile_array {num_pixels} {
	...
	for {set i 0} {$i < $num_pixels} {incr i} {
		...
		for {set j 0} {$j < $num_pixels} {incr j} {
			...
			if {$i == [expr $num_pixels - 1]} {
				...
				port make
				place_coltran $j $pix_width $pix_length $num_pixels
			}
			if {$j == 0} {
			...
			if {$j == [expr $num_pixels - 1]} {
				#wire up gnd	
				box size 3 0.75
				box position [expr $num_pixels * $pix_width] [expr -1 * $i * $pix_length + 0.15]
				paint m1
				
				box size 1 15
				box position [expr $num_pixels * $pix_width+2] [expr -1 * $i * $pix_length + 0.15]
				paint m1
